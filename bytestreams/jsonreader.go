/**
Copyright (c) 2015 The heedy Contributors (see AUTHORS)
...
type JsonReader struct {
	data           pipescript.Iterator // The DataRange to read from
...
func (r *JsonReader) Read(p []byte) (n int, err error) {
	...
	for len(p) > 0 {
		...
		if len(r.currentbuffer) == 0 {
			...
			dp, err := r.data.Next(&pipescript.Datapoint{})
			if err != nil {
				return n, err
			}
...
func NewJsonReader(data pipescript.Iterator, starter, separator, footer, preindent, indent string) (*JsonReader, error) {
	dp, err := data.Next(&pipescript.Datapoint{})
	if err != nil {
		return nil, err
	}
	if dp == nil {
